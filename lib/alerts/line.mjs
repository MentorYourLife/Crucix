...
import { messagingApi, messagingApiBlob, webhook } from '@line/bot-sdk';
import { existsSync, mkdirSync, writeFileSync, readFileSync } from 'fs';
import { join } from 'path';

const { MessagingApiClient } = messagingApi;
const { MessagingApiBlobClient } = messagingApiBlob;
...
export class LineAlerter {
  constructor(config = {}) {
    ...
    if (this.isConfigured) {
      ...
      this.blobClient = new MessagingApiBlobClient({ channelAccessToken: this.channelAccessToken });
    }
      ...
      if (handler) {
        ...
        try {
          ...
          if (typeof response.arrayBuffer === 'function') {
            ...
            console.error('[LINE] Unknown stream type for image.');
            continue;
          }

          const memoryDir = 'c:\\Users\\user\\Desktop\\淬天計畫\\Memory';
          const photosDir = join(memoryDir, 'Photos');
          if (!existsSync(photosDir)) {
            mkdirSync(photosDir, { recursive: true });
          }
          
          const filename = `photo_${Date.now()}_${messageId}.jpg`;
          const filepath = join(photosDir, filename);
          
          writeFileSync(filepath, buffer);
          
          const jsonPath = join(memoryDir, 'photos_record.json');
          let records = [];
          if (existsSync(jsonPath)) {
            try { records = JSON.parse(readFileSync(jsonPath, 'utf8')); } catch(e) {}
          }
          records.push({
            id: messageId,
            filename,
          ...
          if (event.replyToken) {
            await this.client.replyMessage({
              ...
              messages: [this._buildTextMessage(`📸 已收到相片並存入私人大腦：${filename}`)]
            });
          }
        } catch (err) {
          console.error('[LINE] Image handle error:', err);
        }
      }
    }
  }
